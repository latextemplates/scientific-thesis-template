...
- `.latexmkrc` builds every file in a subfolder `core/build`, which keeps the clean folder-hierarchy.
- Added script for creating a new gitignore-file with one command.
- Removed `democode` from gitignore since `build/` has been added and `democode.tex` is moved into `core/build/` automatically, due to `.latexmkrc`.
